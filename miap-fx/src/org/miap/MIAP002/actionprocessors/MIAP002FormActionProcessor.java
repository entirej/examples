...
public class MIAP002FormActionProcessor extends MIAPDefaultFormActionProcessor implements EJFormActionProcessor
{
    ...
    {
        if (record.getBlockName() != null && record.getBlockName().equals(F_MIAP002.B_INVOICE_TOOLBAR.ID))
        {
            ...
            if (F_MIAP002.AC_TOOLBAR_DELETE.equals(command) && form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getFocusedRecord() != null)
            {
                ...
                form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).askToDeleteCurrentRecord();
                return;
            }
        }
        else if (record.getBlockName() != null && record.getBlockName().equals(F_MIAP002.B_INVOICE_POSITIONS_TOOLBAR.ID))
        {
            if (F_MIAP002.AC_TOOLBAR_NEW.equals(command))
            {
            ...
            {
                form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID).askToDeleteCurrentRecord();
                return;
            }
        }
    }
    ...
    public void postDelete(EJForm form, EJRecord record) throws EJActionProcessorException
    {
        ...
        // deleted
        if (F_MIAP002.B_INVOICE_BLOCK.ID.equals(form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getName()))
        {
            ...
            validateToolbarCreateState(form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_TOOLBAR.ID),
                    form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getFocusedRecord() != null);
    ...
    public void validateRecord(EJForm form, EJRecord record, EJRecordType recordType) throws EJActionProcessorException
    {
        ...
        if (F_MIAP002.B_INVOICE_BLOCK.ID.equals(record.getBlockName()))
        {
            ...
            if (recordType == EJRecordType.INSERT || recordType == EJRecordType.UPDATE)
            {
                ...
                final EJScreenItem nrItem = form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_BLOCK.I_NR).getName());
                final EJScreenItem companyNameItem = form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_BLOCK.I_COMPANY_NAME).getName());
                final EJScreenItem customerNameItem = form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_BLOCK.I_CUSTOMER_NAME).getName());
                final EJScreenItem paymentTermsItem = form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_BLOCK.I_PAYMENT_TERMS).getName());
                final EJScreenItem invDateItem = form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_BLOCK.I_INV_DATE).getName());
        ...
        else if (F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID.equals(record.getBlockName()))
        {
            ...
            if (recordType == EJRecordType.INSERT || recordType == EJRecordType.UPDATE)
            {
                ...
                final EJScreenItem prjNameItem = form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_CUST_PROJ_NAME).getName());
                final EJScreenItem vatRateItem = form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_VAT_RATE).getName());
                final EJScreenItem hrsWorkedItem = form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_HOURS_WORKED).getName());
                final EJScreenItem descriptionItem = form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID).getScreenItem(screenType,
                        record.getItem(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_DESCRIPTION).getName());
    ...
    public void lovCompleted(EJForm form, EJScreenItem itemName, boolean arg2) throws EJActionProcessorException
    {
        ...
        if (invPosBlock != null)
        {
            ...
            if (F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_CUST_PROJ_NAME.equals(itemName.getName()))
            {
                ...
                // item
                // due to amount display field being a non block service item
    ...
    public void validateItem(final EJForm form, EJRecord record, String itemName, final EJScreenType screenType) throws EJActionProcessorException
    {
        ...
        if (invPosBlock != null)
        {
            ...
            // if any of the values in the following screen items change then
            // trigger the amount calculation
        ...
        if (record.getBlockName() != null && record.getBlockName().equals(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID))
        {
            ...
            try
            {
                ...
                for (EJRecord invPosRec : form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID).getBlockRecords())
                {
                    ...
                    amtWithVAT += (Double) invPosRec.getValue(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_AMOUNT)
                            + ((Double) invPosRec.getValue(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_AMOUNT) * (Double) invPosRec
                                    .getValue(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.I_VAT_RATE)) / 100;
                }
                ...
                statementExecutor.executeUpdate(form, "INVOICE", criteria, new EJStatementParameter[] {
                        new EJStatementParameter("AMOUNT_EXCL_VAT", Double.class, amount),
                        new EJStatementParameter("AMOUNT_INCL_VAT", Double.class, amtWithVAT) });

                // commit the database transaction
                connection.commit();

                invRec.setValue(F_MIAP002.B_INVOICE_BLOCK.I_AMOUNT_INCL_VAT, amtWithVAT);
                // re-query the invoice block with the last query criteria
                form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).executeLastQuery();
    ...
    {
        // use the focused incvoice record to validate all the toolbar states
        // when
        // entering new record
        EJBlock invoiceBlock = form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID);
        EJRecord invRecord = invoiceBlock.getFocusedRecord();
        ...
        if (record.getBlockName().equals(F_MIAP002.B_INVOICE_BLOCK.ID))
        {
            ...
            validateToolbarState(form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_TOOLBAR.ID), form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID)
                    .getFocusedRecord() != null && invRecord != null);
    ...
    public void postBlockQuery(EJForm form, EJBlock block) throws EJActionProcessorException
    {
        ...
        if (block.getName().equals(F_MIAP002.B_INVOICE_BLOCK.ID))
        {
            ...
            validateToolbarCreateState(form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_TOOLBAR.ID),
                    form.getBlock(F_MIAP002.B_INVOICE_BLOCK.ID).getFocusedRecord() != null && invRecord != null);
            validateToolbarState(form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_TOOLBAR.ID), form.getBlock(F_MIAP002.B_INVOICE_POSITIONS_BLOCK.ID)
                    .getFocusedRecord() != null && invRecord != null);
