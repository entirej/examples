...
public class MIAP001FormActionProcessor extends MIAPDefaultFormActionProcessor implements EJFormActionProcessor
{
    ...
    {
        if (record.getBlockName() != null && record.getBlockName().equals(F_MIAP001.B_CUSTOMER_TOOLBAR.ID))
        {
            ...
            if (F_MIAP001.AC_TOOLBAR_DELETE.equals(command) && form.getBlock(F_MIAP001.B_CUSTOMER_BLOCK.ID).getFocusedRecord() != null)
            {
                ...
                form.getBlock(F_MIAP001.B_CUSTOMER_BLOCK.ID).askToDeleteCurrentRecord();
                return;
            }

        }
        else if (record.getBlockName() != null && record.getBlockName().equals(F_MIAP001.B_CUSTOMER_CONTACT_TOOLBAR.ID))
        {
            if (F_MIAP001.AC_TOOLBAR_NEW.equals(command))
            {
            ...
            {
                form.getBlock(F_MIAP001.B_CUSTOMER_CONTACT_BLOCK.ID).askToDeleteCurrentRecord();
                return;
            }
        }
        else if (record.getBlockName() != null && record.getBlockName().equals(F_MIAP001.B_CUSTOMER_PROJECTS_TOOLBAR.ID))
        {
            if (F_MIAP001.AC_TOOLBAR_NEW.equals(command))
            {
            ...
            if (F_MIAP001.AC_TOOLBAR_DELETE.equals(command) && form.getBlock(F_MIAP001.B_CUSTOMER_PROJECTS_BLOCK.ID).getFocusedRecord() != null)
            {
                ...
                ServiceRetriever.getDBService(form).validateDeleteRecordUsage(form.getBlock(F_MIAP001.B_CUSTOMER_PROJECTS_BLOCK.ID).getFocusedRecord(),
                        "CUSTOMER_PROJECTS");
                form.getBlock(F_MIAP001.B_CUSTOMER_PROJECTS_BLOCK.ID).askToDeleteCurrentRecord();
                return;
            }
        }
    }
    ...
    public void postInsert(EJForm form, EJRecord record) throws EJActionProcessorException
    {
        ...
        {
            // after persisting new customer contact record in the db set the
            // default values
            createDefCustContact(form, record);
        }
        if (form.getBlock(F_MIAP001.B_CUSTOMER_PROJECTS_BLOCK.ID).getName().equals(record.getBlockName()))
        {
            if (record.containsItem(F_MIAP001.B_CUSTOMER_PROJECTS_BLOCK.I_CUST_ID))
            {
                // after persisting new customer project record in the db set
                // the customer id
                EJRecord customerRecord = form.getBlock(F_MIAP001.B_CUSTOMER_BLOCK.ID).getFocusedRecord();
                record.setValue(F_MIAP001.B_CUSTOMER_PROJECTS_BLOCK.I_CUST_ID, customerRecord.getValue(F_MIAP001.B_CUSTOMER_BLOCK.I_ID));
            }
        }
     ...
     */
    private void createDefCustContact(EJForm form, EJRecord record) throws EJActionProcessorException
    {
        ...
        try
        {
            ...
            // all open changes within the form is saved(BlockServiceItem
            // values) except non BlockServiceItem values.
            // Used this to get the FK from CUSTOMER table before saving
            ...
            // query for the main contact type and set it's PK value
        ...
        finally
        {
            ...
            if (error == null)
            {
                // if no errors query the customer block again to load fresh set
                // of data
                form.getBlock(F_MIAP001.B_CUSTOMER_BLOCK.ID).executeQuery();
            }
        }
    ...
    public void postDelete(EJForm form, EJRecord record) throws EJActionProcessorException
    {
        ...
        // deleted
        if (F_MIAP001.B_CUSTOMER_BLOCK.ID.equals(form.getBlock(F_MIAP001.B_CUSTOMER_BLOCK.ID).getName()))
        {
